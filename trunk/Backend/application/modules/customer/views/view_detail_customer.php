...
<script>
    function confirmDelete(idTransaction){
        var url = "<?php echo base_url().'customer/verifycustomer/deletetransaction/'?>"+idTransaction;
 ...
 <div class="col-md-12" id="content">
    ...
    <form role="form" method="get" action="<?php echo base_url().'customer' ;?>" id="search">
       <div class="form-group">
            <div class="input-group date">
              <div class="input-group-addon"><img src="<?php echo base_url() ?>assets/icon_search.png"/></div>
              <input type="text" class="form-control" name="cu_keyword" id="cu_date_keyword" placeholder="<?php if(isset($_GET['cu_keyword'])) echo $_GET['cu_keyword']; else echo 'Search' ?>" >
    ...
    <div class="row" id="content-bottom">
        ...
        <nav>
          <ul class="pagination">
            <li <?php $back = $page - 1; if($back == 0) echo "class='disabled'"; ?>><?php if($back > 0){ echo "<a href='".base_url().'customer/customerdetail/?page='.$back."'>";}?><span aria-hidden="true"><img src="<?php echo base_url().'assets/arrow_page_back.png';?>" /></span><?php if($back > 0){ echo "</a>";} ?></li>
            <?php for($j = $minpage;$j<$maxpage+1;$j++){ ?>
            <li <?php if($j == $page)  echo "class='active'";?>><a href="<?php echo base_url().'customer/customerdetail?page='.$j ; ?>"><?php echo $j; ?></a></li>
            <?php } ?>
            <li <?php $next = $page+1; if($next > $numberpage) echo "class='disabled'"; ?>><?php if($next <= $numberpage){ echo "<a href='".base_url().'customer/customerdetail?page='.$next."'>";}?><span aria-hidden="true"><img src="<?php echo base_url().'assets/arrow_page_next.png';?>" /></span><?php if($next <= $numberpage){ echo "</a>";} ?></li>
          </ul>
        </nav>
    </div> 
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-footer" style="text-align: center;">
                <a id="delete_confirm" href="#"><button type="button" class="btn submitr">YES</button></a>
