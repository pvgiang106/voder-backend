...
<script>
    function confirmDeleteItem(idItem){
        var url = "<?php echo base_url().'admin/verifypartner/deleteitem/'?>"+idItem +"/<?php echo $partner->partnerID ?>";
 ...
 <div class="col-md-12" id="content">
    ...
    <div class="col-md-3 col-md-offset-1" id="div_search">
		<form role="form" method="get" action="<?php echo base_url().'admin/partner/'.$partner->partnerID  ;?>" id="search">
       <div class="form-group">
            <div class="input-group">
              <div class="input-group-addon"><img src="<?php echo base_url() ?>assets/icon_search.png"/></div>
              <input type="text" class="form-control" name="pa_name_keyword" id="pa_name_keyword" placeholder="Search">
              <div class="input-group-addon" id="pa_del_search"></div>
            </div>
        </div>
		</form>
        <a href="<?php echo base_url().'admin/partnertransaction/'.$partner->partnerID ?>" ><button class="btn btn-default">Transaction</button></a>
    </div>        
    </div> 
    ...
    <?php if ($business != null) {?>
       <table class="table table-bordered">
            <tr id="tbl_header">
                <td><b>Menu</b><img data-toggle="modal" data-target="#modalMenuItem" style="margin-left: 712px;;"src="<?php echo base_url().'assets/icon_add_menu.png' ?>" /> New Group <a style="margin-left: 12px; " href="<?php echo base_url().'admin/additem/'.$business->businessID ?>"><img src="<?php echo base_url().'assets/icon_add_menu.png' ?>" /></a> New Item </td>
            </tr>
            <tr>
                <td>
                      ...
                      <div class="panel panel-default">
                        <div class="panel-heading" role="tab" id="<?php echo "heading_".$menu->menuID; ?>">
                          <h4 class="panel-title">
                            ...
                            <a style="width: 300px;" data-toggle="collapse" data-parent="#accordion" href="<?php echo "#collapse_".$menu->menuID; ?>" aria-controls="<?php echo "collapse_".$menu->menuID; ?>">
                           ...
                           <?php $listItem = $menu->listItem ; for($j=0;$j<sizeof($listItem);$j++){ $item = $listItem[$j];?>
                                <tr>
                                    ...
                                    <td style="text-align: center;">
                                        <a href="<?php echo base_url().'admin/updateitem/'.$item->itemID;?>"><img src="<?php echo base_url()?>assets/icon_edit.png"/></a>
...
<div class="modal fade" id="<?php echo 'modal_'.$menu->menuID ?>" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
        ...
        <form role="form" method="post" action="<?php echo base_url().'admin/verifypartner/editmenugroup'?>">
      ...
      <input type="hidden" name="menuID" value="<?php echo $menu->menuID; ?>" />                            
       ...
       <?php } else {?>
            <p style="text-align: center;"><a href="<?php echo base_url().'admin/addbusiness/'.$partner->partnerID ?>"><button class="btn submit">Create business</button></a></p>
       <?php } ?>
    </div>
    <div class="row" id="content-bottom">
        ...
        <nav>
          <ul class="pagination">
            <li <?php $back = $page - 1; if($back == 0) echo "class='disabled'"; ?>><?php if($back > 0){ echo "<a href='".base_url().'admin/partner/'.$partner->partnerID.'?tab_active=1&page='.$back."'>";}?><span aria-hidden="true"><img src="<?php echo base_url().'assets/arrow_page_back.png';?>" /></span><?php if($back > 0){ echo "</a>";} ?></li>
            <?php for($j = $minpage;$j<$maxpage+1;$j++){ ?>
            <li <?php if($j == $page)  echo "class='active'";?>><a href="<?php echo base_url().'admin/partner/'.$partner->partnerID.'?tab_active=1&page='.$j ; ?>"><?php echo $j; ?></a></li>
            <?php } ?>
            <li <?php $next = $page+1; if($next > $numberpage) echo "class='disabled'"; ?>><?php if($next <= $numberpage){ echo "<a href='".base_url().'admin/partner/'.$partner->partnerID.'?tab_active=1&page='.$next."'>";}?><span aria-hidden="true"><img src="<?php echo base_url().'assets/arrow_page_next.png';?>" /></span><?php if($next <= $numberpage){ echo "</a>";} ?></li>
          </ul>
        </nav>
    </div> 
        ...
        <div class="modal fade" id="modalMenuItem" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form role="form" method="post" action="<?php echo base_url().'admin/verifypartner/insertmenugroup'?>">
              <div class="modal-body" >
                <div class="form-group">
                    <label>Group Name</label>
