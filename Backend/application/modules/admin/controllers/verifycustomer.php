...
class VerifyCustomer extends MX_Controller {
        ...
        if (!isset($_POST['email'])) {
            ...
            if ($this->check_email($_POST['email']) == TRUE) {
				...
				if($_FILES["image"]["name"] != ''){
                    ...
                    $target_file = $target_dir.'customer_image_'.$nCustomerID.'.'.$imageFileType;
                    $check = getimagesize($_FILES["image"]["tmp_name"]);
                    if($check) {
                        if(move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)){
                             $this->mdcustomer->updateCustomer($nCustomerID,array('image'=>$target_file));
                        }
    ...
    function updatecustomer($customerID) {
        if (!isset($_POST['customerID'])) {
            ...
            if($_FILES["image"]["name"] != ''){
                ...
                $target_tmp = $target_dir . basename($_FILES["image"]["name"]);
                $imageFileType = pathinfo($target_tmp,PATHINFO_EXTENSION);
                $target_file = $target_dir.'customer_image_'.$customerID.'.'.$imageFileType;
                $check = getimagesize($_FILES["image"]["tmp_name"]);
                if($check) {
                    if(move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)){
    ...
    function deletetransaction($transactionID,$customerID){        
        ...
        $ID = $transactionID;
        $this->mdcustomer->inactiveRecord($tableName,$nameID,$ID);
        redirect(base_url().'admin/customer/'.$customerID);
    }
}
